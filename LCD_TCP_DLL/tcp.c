...
int TcpInit(uint32_t ipAddr, uint16_t portNum)
{
    ...
    WSADATA data;
    if (WSAStartup(sockVersion, &data) != 0) return -1;
    ...
    // --- FIX FOR ESP8266 STABILITY ---
    // Previously: value = 1 (Enabled). 
    // New: value = 0 (Disabled). 
    // This allows Nagle's algorithm to bundle small packets, reducing CPU load on the ESP.
    DWORD value = 0;
    setsockopt(sclient, IPPROTO_TCP, TCP_NODELAY, (char*)&value, sizeof(value));
...
int TcpHasData(int timeoutMs)
{
    ...
    if (ret > 0) return 1; // Data available
